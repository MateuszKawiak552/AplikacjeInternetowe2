...
final class MeasurementController extends AbstractController
{
    ...
    #[Route('/{city}', name: 'app_measurement_by_city', requirements: ['city' => '[a-zA-Z]+'])]
    #[IsGranted('ROLE_MEASUREMENT_CITY')]
    public function city(string $city, MeasurementRepository $measurementRepository, LocalizationRepository $localizationRepository): Response
    {
        $localization = $localizationRepository->findByCity($city);
        $measurements = $measurementRepository->findByLocalization($localization);

        return $this->render('measurement/city.html.twig', [
